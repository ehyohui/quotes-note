...
const DEFAULT_SOURCES = [“롱블랙”, “민음사 세계문학일력”, “밀리의 서재”, “퍼블리”];
...
function AddModal({ onClose, onAdd, onEdit, existingQuotes, initialData, sources }) {
const isEdit = !!initialData;
const [texts, setTexts]   = useState(isEdit ? [initialData.text] : [””]);
const [source, setSource] = useState(initialData?.source || sources[0] || “롱블랙”);
...
{!isEdit && <button onClick={()=>setTexts(p=>[…p,””])} style={{ fontSize:“18px”, color:C.terracotta, background:“none”, border:“none”, cursor:“pointer”, fontWeight:“400”, lineHeight:1 }}>+</button>}
</div>
{texts.map((t,i) => (
...
function Drawer({ open, onClose, activeSource, setActiveSource, counts, sources, onAddSource, onDeleteSource }) {
...
<div style={{ position:“absolute”, left:0, top:0, bottom:0, width:220, background:C.cream, padding:“24px 14px 28px”, overflowY:“auto” }}>
...
```
    <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:8 }}>
      <button onClick={()=>setShowInput(v=>!v)}
        style={{ fontSize:"16px", color:C.terracotta, background:"none", border:"none", cursor:"pointer", fontWeight:"400", lineHeight:1, padding:"2px 4px" }}>
        +
      </button>
    </div>
    ...
    {showInput && (
      <div style={{ marginBottom:6, position:"relative" }}>
        <input value={newName} onChange={e=>setNewName(e.target.value)}
          ...
          placeholder="출처 이름" autoFocus
          style={{ width:"100%", padding:"8px 40px 8px 10px", border:"1px solid "+C.border, borderRadius:"8px", fontSize:"13px", color:C.text, background:C.white, outline:"none", boxSizing:"border-box" }}
          onFocus={e=>(e.target.style.borderColor=C.terracotta)}
          onBlur={e=>(e.target.style.borderColor=C.border)} />
        <button onClick={handleAdd}
          style={{ position:"absolute", right:6, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:C.terracotta, fontSize:"16px", fontWeight:"600", padding:"2px 4px", lineHeight:1 }}>✓</button>
      </div>
    )}

    <button onClick={()=>{setActiveSource("전체");onClose();}}
      style={{ display:"flex", alignItems:"center", width:"100%", padding:"8px 10px", borderRadius:"8px", border:"none", background:activeSource==="전체"?C.terracotta:"transparent", cursor:"pointer", fontSize:"14px", color:activeSource==="전체"?"#fff":C.muted, fontWeight:activeSource==="전체"?"600":"400", marginBottom:2, textAlign:"left", gap:8 }}>
      <span style={{width:8}}/><span style={{flex:1}}>전체</span>
      <span style={{fontSize:"12px",opacity:0.7}}>{counts["전체"]??0}</span>
    </button>

    {sources.map(s=>(
      <div key={s} style={{ display:"flex", alignItems:"center", gap:2, marginBottom:2 }}>
        <button onClick={()=>{setActiveSource(s);onClose();}}
          style={{ display:"flex", alignItems:"center", flex:1, padding:"8px 10px", borderRadius:"8px", border:"none", background:activeSource===s?C.terracotta:"transparent", cursor:"pointer", fontSize:"14px", color:activeSource===s?"#fff":C.muted, fontWeight:activeSource===s?"600":"400", textAlign:"left", gap:8 }}>
          <SourceDot source={s}/><span style={{flex:1}}>{s}</span>
          <span style={{fontSize:"12px",opacity:0.7}}>{counts[s]??0}</span>
        </button>
        <button onClick={e=>{e.stopPropagation();onDeleteSource(s);}}
          style={{ background:"none", border:"none", cursor:"pointer", color:C.muted, fontSize:"18px", padding:"6px 8px", borderRadius:"6px", flexShrink:0, lineHeight:1 }}
          onMouseEnter={e=>(e.currentTarget.style.color=C.terracotta)}
          onMouseLeave={e=>(e.currentTarget.style.color=C.muted)}>×</button>
      </div>
    ))}
...
try {
const saved = localStorage.getItem(“sources-data”);
...
const saveQuotes = (q) => {
setQuotes(q);
try { localStorage.setItem(“quotes-data”, JSON.stringify(q)); } catch (e) { console.error(“save error”, e); }
};

const saveSources = (s) => {
setSources(s);
try { localStorage.setItem(“sources-data”, JSON.stringify(s)); } catch {}
};

const handleAddSource = () => {
const name = newSourceName.trim();
if (!name || sources.includes(name)) return;
saveSources([…sources, name]);
setNewSourceName(””);
setShowAddSource(false);
};

const handleDeleteSource = (s) => {
if (!window.confirm(`"${s}" 출처를 삭제할까요?`)) return;
saveSources(sources.filter(x => x !== s));
if (activeSource === s) setActiveSource(“전체”);
};
...
sources.forEach(s=>{c[s]=quotes.filter(q=>q.source===s).length;});
return c;
}, [quotes, sources]);
...
```
  {!isMobile && (
    <aside style={{ width:210, flexShrink:0, background:C.cream, borderRight:`1px solid ${C.border}`, padding:"28px 14px", position:"sticky", top:0, height:"100vh", overflowY:"auto" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
        <span style={{ fontSize:"10px", color:C.muted, letterSpacing:"0.1em", textTransform:"uppercase" }}>출처</span>
      ...
      {showAddSource && (
        <div style={{ marginBottom:8, position:"relative" }}>
          <input value={newSourceName} onChange={e=>setNewSourceName(e.target.value)}
            ...
            placeholder="출처 이름" autoFocus
            style={{ width:"100%", padding:"8px 36px 8px 10px", border:`1px solid ${C.border}`, borderRadius:"8px", fontSize:"13px", color:C.text, background:C.white, outline:"none", boxSizing:"border-box" }}
            onFocus={e=>(e.target.style.borderColor=C.terracotta)}
            onBlur={e=>(e.target.style.borderColor=C.border)} />
          <button onClick={handleAddSource}
            style={{ position:"absolute", right:8, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:C.terracotta, fontSize:"16px", fontWeight:"600", padding:"2px 2px", lineHeight:1 }}>✓</button>
        </div>
      )}

      {/* 전체 버튼 */}
      <button onClick={()=>setActiveSource("전체")}
        onMouseEnter={e=>{if(activeSource!=="전체")e.currentTarget.style.background=C.hover;}}
        onMouseLeave={e=>{if(activeSource!=="전체")e.currentTarget.style.background="transparent";}}
        style={{ display:"flex", alignItems:"center", width:"100%", padding:"6px 8px", borderRadius:"6px", border:"none", background:activeSource==="전체"?C.terracotta:"transparent", cursor:"pointer", fontSize:"13px", color:activeSource==="전체"?"#fff":C.muted, fontWeight:activeSource==="전체"?"600":"400", marginBottom:2, textAlign:"left", gap:6, transition:"all 0.15s" }}>
        <span style={{width:8}}/>
        <span style={{flex:1}}>전체</span>
        <span style={{fontSize:"11px",opacity:0.7}}>{counts["전체"]??0}</span>
      </button>

      {/* 출처 목록 + 삭제 버튼 */}
      {sources.map(s=>(
        <div key={s} style={{ display:"flex", alignItems:"center", gap:2, marginBottom:2 }}>
          <button onClick={()=>setActiveSource(s)}
            ...
            style={{ display:"flex", alignItems:"center", flex:1, padding:"6px 8px", borderRadius:"6px", border:"none", background:activeSource===s?C.terracotta:"transparent", cursor:"pointer", fontSize:"13px", color:activeSource===s?"#fff":C.muted, fontWeight:activeSource===s?"600":"400", textAlign:"left", gap:6, transition:"all 0.15s" }}>
            <SourceDot source={s}/>
            <span style={{flex:1}}>{s}</span>
            <span style={{fontSize:"11px",opacity:0.7}}>{counts[s]??0}</span>
          </button>
          <button onClick={e=>{e.stopPropagation();handleDeleteSource(s);}}
            style={{ background:"none", border:"none", cursor:"pointer", color:C.muted, fontSize:"18px", padding:"6px 8px", borderRadius:"6px", flexShrink:0, lineHeight:1 }}
            onMouseEnter={e=>(e.currentTarget.style.color=C.terracotta)}
            onMouseLeave={e=>(e.currentTarget.style.color=C.muted)}>×</button>
        </div>
  ...
  <Drawer open={showDrawer} onClose={()=>setShowDrawer(false)} activeSource={activeSource} setActiveSource={setActiveSource} counts={counts} sources={sources} onAddSource={name=>{saveSources([...sources,name]);}} onDeleteSource={handleDeleteSource} />
  ...
  <main style={{ flex:1, padding:isMobile?"16px 16px 40px":"32px 40px", maxWidth:860, overflowX:"hidden" }}>
            ...
            <div style={{ display:"flex", gap:16 }}>
              <div style={{ display:"flex", flexDirection:"column", gap:4, flex:1 }}>
                ...
                <div style={{ position:"relative", height:26 }}>
                  <span style={{ position:"absolute", left:0, bottom:4, fontSize:"13px", color:dateFrom?"transparent":"#C8B8A8", pointerEvents:"none" }}>날짜 선택</span>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4, flex:1 }}>
                ...
                <div style={{ position:"relative", height:26 }}>
                  <span style={{ position:"absolute", left:0, bottom:4, fontSize:"13px", color:dateTo?"transparent":"#C8B8A8", pointerEvents:"none" }}>날짜 선택</span>
      ...
      {FIXED_TAGS.map(t=>{
        ...
        return (
          <button key={t} onClick={()=>{setActiveTag(on?"":t);setPage(1);}}
            style={{ padding:"4px 11px", borderRadius:"20px", cursor:"pointer", fontSize:"12px", border:on?`1.5px solid ${C.terracotta}`:`1.5px solid ${C.border}`, background:on?C.terracotta:C.pill, color:on?"#fff":C.pillText, fontWeight:on?"700":"500", boxShadow:on?"0 2px 8px rgba(196,82,42,0.25)":"0 1px 3px rgba(44,26,14,0.08)", transition:"all 0.15s" }}>
    ...
    <div style={{ display:"flex", justifyContent:"center", marginTop:16, paddingBottom:8 }}>
      <button onClick={()=>setShowModal(true)}
        style={{ width:"40%", padding:"10px", background:C.terracotta, border:"none", borderRadius:"10px", color:"#fff", fontSize:"15px", fontWeight:"600", cursor:"pointer", boxShadow:"0 2px 8px rgba(196,82,42,0.3)", letterSpacing:"0.02em" }}
  ...
  {showModal && <AddModal onClose={()=>setShowModal(false)} onAdd={q=>saveQuotes([...q,...quotes])} onEdit={()=>{}} existingQuotes={quotes} sources={sources} />}
  {editTarget && <AddModal onClose={()=>setEditTarget(null)} onAdd={()=>{}} onEdit={u=>{saveQuotes(quotes.map(q=>q.id===u.id?u:q));setEditTarget(null);}} existingQuotes={quotes} initialData={editTarget} sources={sources} />}
</div>
```
